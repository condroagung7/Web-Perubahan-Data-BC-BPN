...
export async function POST(request: NextRequest) {
  ...
  if (dokumenPaths.length > 0) {
    ...
    if (removeDokumenError) {
      console.error("Gagal menghapus dokumen pendukung:", removeDokumenError);
      return NextResponse.json({ error: "Gagal menghapus dokumen pendukung" }, { status: 500 });
    }
  }
  ...
  if (permohonan.surat_persetujuan_url) {
    ...
    if (removeSuratError) {
      ...
      return NextResponse.json({ error: "Gagal menghapus surat persetujuan" }, { status: 500 });
    }
  }

  const { error: deleteError } = await supabase.from("permohonan").delete().eq("id", id);

  if (deleteError) {
    console.error("Gagal menghapus data permohonan:", deleteError);
    return NextResponse.json({ error: "Gagal menghapus data permohonan" }, { status: 500 });
  }

  return NextResponse.json({ message: "Permohonan berhasil dihapus dari dashboard" });
}
