...
export const POST = secureHandler(
  async (request: NextRequest) => {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Status seksi tidak valid" },
        { status: 400 }
      );
    }
    ...
    if (body.status && !STATUS_PERMOHONAN_VALUES.includes(body.status)) {
      return NextResponse.json(
        { error: "Status permohonan tidak valid" },
        { status: 400 }
      );
    }
    ...
    if ("id_nadine" in body)
      updateData.id_nadine = body.id_nadine
        ? sanitizeString(body.id_nadine).trim() || null
        : null;
    if ("pic" in body)
      updateData.pic = body.pic
        ? sanitizeString(body.pic).trim() || null
        : null;
    if ("status" in body && body.status) updateData.status = body.status;
    if ("status_seksi" in body)
      updateData.status_seksi = body.status_seksi ?? null;
    ...
    // Audit log
    logAdminAction("update_meta", user.email ?? "unknown", {
      targetId: id,
