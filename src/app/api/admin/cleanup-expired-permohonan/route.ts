...
async function deletePermohonanFiles(permohonan: ExpiredPermohonan) {
  ...
  if (dokumenPaths.length > 0) {
    const { error } = await supabase.storage.from(DOKUMEN_BUCKET).remove(dokumenPaths);
  ...
  if (permohonan.surat_persetujuan_url) {
    ...
    const { error } = await supabase.storage.from(SURAT_BUCKET).remove([suratPath]);
    if (error) throw error;
  }
}
...
export async function GET(request: NextRequest) {
  ...
  if (riwayatError) {
    ...
    return NextResponse.json({ error: "Gagal membaca riwayat status" }, { status: 500 });
  }

  const expiredIds = Array.from(
    new Set((riwayatList ?? []).map((item) => item.permohonan_id).filter(Boolean))
  );

  if (expiredIds.length === 0) {
    return NextResponse.json({
      message: "Tidak ada permohonan diterima lengkap yang melewati 30 hari",
      deleted: 0,
    });
  }
  ...
  if (permohonanError) {
    ...
    return NextResponse.json({ error: "Gagal membaca data permohonan" }, { status: 500 });
  }
  ...
  for (const permohonan of permohonanList ?? []) {
    try {
      ...
      console.error(`Gagal menghapus permohonan expired ${permohonan.id}:`, error);
      failed.push({
        id: permohonan.id,
        error: error instanceof Error ? error.message : "Gagal menghapus permohonan",
      });
    }
  }
