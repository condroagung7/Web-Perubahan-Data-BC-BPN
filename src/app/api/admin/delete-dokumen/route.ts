...
export async function POST(request: NextRequest) {
  ...
  if (removeError) {
    ...
    return NextResponse.json({ error: "Gagal menghapus file dokumen" }, { status: 500 });
  }

  const nextDokumen = currentDokumen.filter((dokumen) => dokumen.url !== path);
  ...
  return NextResponse.json({ message: "Dokumen pendukung berhasil dihapus" });
}
