...
export async function POST(request: NextRequest) {
  ...
  if (updateError) {
    console.error("Gagal memperbarui data berkas permohonan:", updateError);
    return NextResponse.json(
      { error: "File terhapus, tetapi data permohonan gagal diperbarui" },
      { status: 500 }
    );
  }

  return NextResponse.json({
    message: "Surat persetujuan dan dokumen pendukung berhasil dihapus",
  });
}
